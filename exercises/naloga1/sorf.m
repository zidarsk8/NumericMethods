...
function [Z,iter,err] = sorj2w(f,Z,x,y,h,w,maxIter,maxError)
    s = size(Z);
    prev = rand(s);
    iter = 0;
    err = [maxError + 1];
    ...
    ff11 = ff(a1-1,b1-1);
    while iter++ < maxIter && err(end) > maxError
        Z(a2,b1) = (1-w).*Z(a2,b1) + w .*  ( Z(a2+1,b1)+Z(a2-1,b1)+Z(a2,b1+1)+Z(a2,b1-1) - ff21)./4;
        Z(a1,b2) = (1-w).*Z(a1,b2) + w .*  ( Z(a1+1,b2)+Z(a1-1,b2)+Z(a1,b2+1)+Z(a1,b2-1) - ff12)./4;
        Z(a1,b1) = (1-w).*Z(a1,b1) + w .*  ( Z(a1+1,b1)+Z(a1-1,b1)+Z(a1,b1+1)+Z(a1,b1-1) - ff11)./4;
        Z(a2,b2) = (1-w).*Z(a2,b2) + w .*  ( Z(a2+1,b2)+Z(a2-1,b2)+Z(a2,b2+1)+Z(a2,b2-1) - ff22)./4;
        err = [err max(max(abs(Z-prev)))];
        prev = Z;
    endwhile
endfunction
...
function sorj2Test(m,n,maxIter,maxError)
    ...
    f = @(x,y)-ifelse(y==(hal*h),3,0).*ifelse(x==(hal*h),3,0);
    tic;
    [A, iter, err] = sorj2(f,Z,a,b,h,maxIter,0);
    t = toc;
    printf ("\niteracije:   %d \n", iter);
    printf ("napaka:      %e \n", err(end));
    printf ("cas:         %f \n\n", t);
...
function sorj2TestW(m,n,a,b,h,fun,u,steps)
    Z = genZ(m,n,a,b,h,u);
    maxIter = 10000;
    maxError = 0;
    ...
    for i=1:steps
        f1 = f + (t-f)/3;
        t1 = f + (t-f)/3*2;
        [Af, iterf, errf] = sorj2w(fun,Z,a,b,h,f1,maxIter,maxError);
        [At, titert, errt] = sorj2w(fun,Z,a,b,h,t1,maxIter,maxError);
        ef = max(max(abs(A-Af)));
        et = max(max(abs(A-At)));
        if ef < et
            t = t1;
